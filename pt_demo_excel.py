...
#actTypes.reverse() # list of Activity Types

## Generate a set of widsgets from the excel data
testType = actTypes[0]
testCat = 'Value For Organisation'
testQuestions = []
temp = []

for i in range(numQuestions):
	at = importedData.iloc[i]['ACTIVITY_TYPE']
	cat = importedData.iloc[i]['CATEGORY']
	print(cat)
	if (at == testType) and (cat == testCat):
		temp = [importedData.iloc[i]['QUESTION'],importedData.iloc[i]['QUESTION_TYPE'],importedData.iloc[i]['WEIGHT'], importedData.iloc[i]['CATEGORY'],keys[i]]
		testQuestions.append(temp)

testResult = [len(testQuestions)] # holder for data
testCont = st.container(border=True)

with testCont.container():
	headertext = testCat + " - " + testType
	st.header(headertext)
	for q in testQuestions:
		if q[1] == 'binary':
			st.radio(q[0],['YES', 'NO'], horizontal=True, key = q[4])
		elif q[1] == 'numeric':
			st.number_input(q[0], step = 1, key = q[4])
